// Reference
// 1. S. Izvekov, B. M. Rice, Multi-scale coarse-graining of non-conservative interactions in molecular liquids, J. Chem. Phys., 140, 104104 (2014)
// 2. C. Junghans, M. Praprotnik and K. Kremer, Transport properties controlled by a thermostat: An extended dissipative particle dynamics thermostat, Soft Matter, 2008, 4, 156-161
...
PairTableDPD::PairTableDPD(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
  random = NULL;
...
void PairTableDPD::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double vxtmp,vytmp,vztmp,delvx,delvy,delvz;
  double randnum_para, randnum_vertical1, randnum_vertical2;
  double rsq,factor_lj,fraction,value,a,b,invr;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (tabstyle == LOOKUP) {
          ...
          double parallelGamma = tb->p[itable];
          double verticalGamma = tb->v[itable];
          double v_para = (delvx * delx + delvy * dely + delvz * delz) / rsq;
          double v_vetx = delvx - v_para * delx;
          double v_vety = delvy - v_para * dely;
          double v_vetz = delvz - v_para * delz;
          double friction = parallelGamma * v_para;
          double fluc = 2.0 * kT *parallelGamma;
          double rdforce = 0.0;
          if (fluc > 1e-30)
            rdforce = sqrt(fluc);
      
          randnum_para = random->gaussian();
          fpair = factor_lj * value - friction + randnum_para * rdforce * dtinvsqrt * invr; //Double Check This!

          //vertical vector 1
          randnum_vertical1 = random->gaussian();
          double r1vx = 0;
          double r1vy = sqrt(1.0 / (1.0 + (dely / delz) * (dely / delz)));
          double r1vz = - (dely / delz) * r1vy;
          fluc = 2.0 * kT * verticalGamma;
          rdforce = (fluc > 1e-30) ? sqrt(fluc) : 0.0;
          double randomv1 = randnum_vertical1 * rdforce * dtinvsqrt;  //Double Check

	  randnum_vertical2 = random->gaussian();
          double r2vx = (dely * r1vz - delz * r1vy) * invr;
          double r2vy = -delx * r1vz * invr;
          double r2vz = delx * r1vy * invr;
          double randomv2 = randnum_vertical2 * rdforce * dtinvsqrt;  //Double Check

          f[i][0] += (delx*fpair - verticalGamma*v_vetx + randomv2 * r2vx + randomv1 * r1vx);
          f[i][1] += (dely*fpair - verticalGamma*v_vety + randomv2 * r2vy + randomv1 * r1vy);
          f[i][2] += (delz*fpair - verticalGamma*v_vetz + randomv2 * r2vz + randomv1 * r1vz);
          if (newton_pair || j < nlocal) {
            f[j][0] -= (delx*fpair - verticalGamma*v_vetx + randomv2 * r2vx + randomv1 * r1vx);
            f[j][1] -= (dely*fpair - verticalGamma*v_vety + randomv2 * r2vy + randomv1 * r1vy);
            f[j][2] -= (delz*fpair - verticalGamma*v_vetz + randomv2 * r2vz + randomv1 * r1vz);
          }
          ...
          double parallelGamma = tb->p[itable] + fraction*tb->dp[itable];
          double verticalGamma = tb->v[itable] + fraction*tb->dv[itable];
          double v_para = (delvx * delx + delvy * dely + delvz * delz) / rsq;
          double v_vetx = delvx - v_para * delx;
          double v_vety = delvy - v_para * dely;
          double v_vetz = delvz - v_para * delz;
	  ...
	  randnum_vertical2 = random->gaussian();
          ...
          double parallelGamma = a * tb->p[itable] + b * tb->p[itable+1] + ((a*a*a - a)*tb->p2[itable] + (b*b*b-b)*tb->p2[itable+1]) * tb->deltasq6;
          double verticalGamma = a * tb->v[itable] + b * tb->v[itable+1] + ((a*a*a - a)*tb->v2[itable] + (b*b*b-b)*tb->v2[itable+1]) * tb->deltasq6;
          double v_para = (delvx * delx + delvy * dely + delvz * delz) / rsq;
          double v_vetx = delvx - v_para * delx;
          double v_vety = delvy - v_para * dely;
          double v_vetz = delvz - v_para * delz;
	  ...
	  randnum_vertical2 = random->gaussian();
        ...
        if (langevinFriction > 0)
        {
          f[i][0] += -langevinFriction * v[i][0] + sqrt(2.0 * kT * langevinFriction) * random->gaussian() * dtinvsqrt;
          f[i][1] += -langevinFriction * v[i][1] + sqrt(2.0 * kT * langevinFriction) * random->gaussian() * dtinvsqrt;
          f[i][2] += -langevinFriction * v[i][2] + sqrt(2.0 * kT * langevinFriction) * random->gaussian() * dtinvsqrt;
        }
...
void PairTableDPD::settings(int narg, char **arg)
{
  if (narg < 3) error->all(FLERR,"Illegal pair_style command");

  // new settings
  T = atof(arg[0]);
  langevinFriction = atof(arg[1]);
  //printf("temperature = %lf\n", T);
  random = new RanMars(lmp, 111208+comm->me);

  if (strcmp(arg[2],"lookup") == 0) tabstyle = LOOKUP;
  else if (strcmp(arg[2],"linear") == 0) tabstyle = LINEAR;
  else if (strcmp(arg[2],"spline") == 0) tabstyle = SPLINE;
  else if (strcmp(arg[2],"bitmap") == 0) tabstyle = BITMAP;
  else error->all(FLERR,"Unknown table style in pair_style command");

  tablength = force->inumeric(FLERR,arg[3]);
...
void PairTableDPD::coeff(int narg, char **arg)
{
...
double PairTableDPD::init_one(int i, int j)
{
...
/* ----------------------------------------------------------------------
   ...
   this function sets these values in Table:
     ninput,rfile,efile,ffile,pfile,vfile,rflag,rlo,rhi,fpflag,fplo,fphi,ntablebits
------------------------------------------------------------------------- */

void PairTableDPD::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
  ...
  memory->create(tb->pfile,tb->ninput,"pair:pfile");
  memory->create(tb->vfile,tb->ninput,"pair:vfile");
  ...
  // read r,e,f,p,v table values from file
  ...
  for (int i = 0; i < tb->ninput; i++) {
    fgets(line,MAXLINE,fp);
    sscanf(line,"%d %lg %lg %lg %lg %lg",&itmp,&rtmp,&tb->efile[i],&tb->ffile[i], &tb->pfile[i], &tb->vfile[i]);
    printf("%d %lg %lg %lg %lg %lg\n",itmp,rtmp,tb->efile[i],tb->ffile[i], tb->pfile[i], tb->vfile[i]);
...
void PairTableDPD::bcast_table(Table *tb)
{
  ...
  MPI_Bcast(tb->pfile,tb->ninput,MPI_DOUBLE,0,world);
  MPI_Bcast(tb->vfile,tb->ninput,MPI_DOUBLE,0,world);
...
/* ----------------------------------------------------------------------
   build spline representation of e,f,p,v over entire range of read-in table
   this function sets these values in Table: e2file,f2file
------------------------------------------------------------------------- */

void PairTableDPD::spline_table(Table *tb)
{
  memory->create(tb->e2file,tb->ninput,"pair:e2file");
  memory->create(tb->f2file,tb->ninput,"pair:f2file");
  memory->create(tb->p2file,tb->ninput,"pair:p2file");
  memory->create(tb->v2file,tb->ninput,"pair:v2file");

  double ep0 = - tb->ffile[0];
  double epn = - tb->ffile[tb->ninput-1];
  printf("ep0 = %lf epn = %lf\n", ep0, epn);
  ...
  if (tb->fpflag == 0) {
    ...
    //Extension for p and v
    tb->pplo = (tb->pfile[1] - tb->pfile[0]) / (tb->rfile[1] - tb->rfile[0]);
    tb->pphi = (tb->pfile[tb->ninput-1] - tb->pfile[tb->ninput-2]) /
      (tb->rfile[tb->ninput-1] - tb->rfile[tb->ninput-2]);

    tb->vplo = (tb->vfile[1] - tb->vfile[0]) / (tb->rfile[1] - tb->rfile[0]);
    tb->vphi = (tb->vfile[tb->ninput-1] - tb->vfile[tb->ninput-2]) /
      (tb->rfile[tb->ninput-1] - tb->rfile[tb->ninput-2]);
  }
  ...
  double pp0 = tb->pplo;
  double ppn = tb->pphi;
  spline(tb->rfile,tb->pfile,tb->ninput,pp0,ppn,tb->p2file);

  double vp0 = tb->vplo;
  double vpn = tb->vphi;
  spline(tb->rfile,tb->vfile,tb->ninput,vp0,vpn,tb->v2file);
}
...
void PairTableDPD::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTableDPD::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
  ...
  if (tabstyle == LOOKUP) {
    ...
    for (int i = 0; i < tlm1; i++) {
      ...
      tb->p[i] = splint(tb->rfile,tb->pfile,tb->p2file,tb->ninput,r);
      tb->v[i] = splint(tb->rfile,tb->vfile,tb->v2file,tb->ninput,r);
    }
  }
  ...
  if (tabstyle == LINEAR) {
    ...
    memory->create(tb->p,tablength,"pair:p");
    memory->create(tb->v,tablength,"pair:v");
    memory->create(tb->de,tlm1,"pair:de");
    memory->create(tb->df,tlm1,"pair:df");
    memory->create(tb->dp,tlm1,"pair:dp");
    memory->create(tb->dv,tlm1,"pair:dv");
    ...
    for (int i = 0; i < tablength; i++) {
      ...
      printf("e[%d] = %lf f = %lf p = %lf v = %lf\n", i, tb->e[i], tb->f[i], tb->p[i], tb->v[i]);
    }

    for (int i = 0; i < tlm1; i++) {
      tb->de[i] = tb->e[i+1] - tb->e[i];
      tb->df[i] = tb->f[i+1] - tb->f[i];
      tb->dp[i] = tb->p[i+1] - tb->p[i];
      tb->dv[i] = tb->v[i+1] - tb->v[i];
    }
  }
  ...
  if (tabstyle == SPLINE) {
    ...
    memory->create(tb->p2,tablength,"pair:p2");
    memory->create(tb->v2,tablength,"pair:v2");
    ...
    // pp0, ppn = dh / dg at inner and at cut
    // h(r) = v(r) and g(r) = r^2
    // dh / dg in secant approx = (h(r2) - h(r1)) / (g(r2) - g(r1))
    double pp0, ppn;
    double rsq1 = tb->cut * tb->cut;
    double rsq2 = rsq1 + secant_factor * tb->delta;
    pp0 = (splint(tb->rfile,tb->pfile,tb->p2file,tb->ninput,sqrt(rsq2)) - tb->p[0]) / (secant_factor * tb->delta);
    rsq2 = tb->cut * tb->cut;
    rsq1 = rsq2 - secant_factor * tb->delta;
    ppn = (tb->p[tlm1] - splint(tb->rfile,tb->pfile,tb->p2file,tb->ninput,sqrt(rsq1))) / (secant_factor * tb->delta);
    spline(tb->rsq,tb->p,tablength,pp0,ppn,tb->p2);

    double vp0, vpn;
    //double rsq1 = tb->cut * tb->cut;
    //double rsq2 = rsq1 + secant_factor * tb->delta;
    rsq1 = tb->cut * tb->cut;
    rsq2 = rsq1 + secant_factor * tb->delta;
    vp0 = (splint(tb->rfile,tb->vfile,tb->v2file,tb->ninput,sqrt(rsq2)) - tb->v[0]) / (secant_factor * tb->delta);
    rsq2 = tb->cut * tb->cut;
    rsq1 = rsq2 - secant_factor * tb->delta;
    ppn = (tb->v[tlm1] - splint(tb->rfile,tb->vfile,tb->v2file,tb->ninput,sqrt(rsq1))) / (secant_factor * tb->delta);
    spline(tb->rsq,tb->v,tablength,vp0,vpn,tb->v2);
  }
  ...
  if (tabstyle == BITMAP) {
    ...
    memory->create(tb->p,ntable,"pair:p");
    memory->create(tb->v,ntable,"pair:v");
    memory->create(tb->de,ntable,"pair:de");
    memory->create(tb->df,ntable,"pair:df");
    memory->create(tb->dp,ntable,"pair:dp");
    memory->create(tb->dv,ntable,"pair:dv");
    ...
    tb->dp[ntablem1] = tb->p[0] - tb->p[ntablem1];
    tb->dv[ntablem1] = tb->v[0] - tb->v[ntablem1];
    ...
    double e_tmp,f_tmp,p_tmp,v_tmp;
    ...
    if (rsq_lookup.f < tb->cut*tb->cut) {
      if (tb->match) {
        ...
        tb->dp[itablemax] = tb->dp[itablemaxm1];
        tb->dv[itablemax] = tb->dv[itablemaxm1];
        ...
        p_tmp = splint(tb->rfile,tb->pfile,tb->p2file,tb->ninput,r);
        v_tmp = splint(tb->rfile,tb->vfile,tb->v2file,tb->ninput,r);
        tb->de[itablemax] = e_tmp - tb->e[itablemax];
        tb->df[itablemax] = f_tmp - tb->f[itablemax];
        tb->dp[itablemax] = p_tmp - tb->p[itablemax];
        tb->dv[itablemax] = v_tmp - tb->v[itablemax];
...
void PairTableDPD::null_table(Table *tb)
{
  tb->rfile = tb->efile = tb->ffile = tb->pfile = tb->vfile = NULL;
  tb->e2file = tb->f2file = tb->p2file = tb->v2file = NULL;
  tb->rsq = tb->drsq = tb->e = tb->de = NULL;
  tb->f = tb->df = tb->e2 = tb->f2 = tb->p = tb->v = tb->dp = tb->dv = tb->e2 = tb->f2 = NULL;
}
...
void PairTableDPD::free_table(Table *tb)
{
  ...
  memory->destroy(tb->pfile);
  memory->destroy(tb->vfile);
  memory->destroy(tb->p2file);
  memory->destroy(tb->v2file);
  ...
  memory->destroy(tb->p);
  memory->destroy(tb->dp);
  memory->destroy(tb->v);
  memory->destroy(tb->dv);
  memory->destroy(tb->p2);
  memory->destroy(tb->v2);
}
...
void PairTableDPD::spline(double *x, double *y, int n,
...
double PairTableDPD::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTableDPD::write_restart(FILE *fp)
{
...
void PairTableDPD::read_restart(FILE *fp)
{
...
void PairTableDPD::write_restart_settings(FILE *fp)
{
...
void PairTableDPD::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTableDPD::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairTableDPD::extract(const char *str, int &dim)
{
